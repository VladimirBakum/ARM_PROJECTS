...
static void loadHeader() 
{
 ...
 // Offset 1080
 f_read(&file, temp, 4, &count); // читаем 4 байта
...
static void loadPattern(uint8_t pattern) 
{
 ...
 f_lseek(&file, 1084 + pattern * ROWS * uMod.Mod.numberOfChannels * 4); // Новый указатель файла
...
static void processRow() {
 ...
 for(channel = 0; channel < uMod.Mod.numberOfChannels; channel++) {
  ...
  if(note != NONOTE_MOD) {
   ...
   if(effectNumber != TONEPORTAMENTO_MOD && effectNumber != PORTAMENTOVOLUMESLIDE_MOD)
  ...
  switch(effectNumber) {
   ...
   case PORTAMENTOVOLUMESLIDE_MOD:
    uPlayer.Mod_player.portamentoNote[channel] = uPlayer.Mod_player.amigaPeriod[channel];
    note = NONOTE_MOD;
    break;
  ...
  if(note != NONOTE_MOD || uPlayer.Mod_player.lastAmigaPeriod[channel] &&
     effectNumber != VIBRATO_MOD && effectNumber != VIBRATOVOLUMESLIDE_MOD &&
