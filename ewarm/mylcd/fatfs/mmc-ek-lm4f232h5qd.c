...
void power_on (void)
{
    ...
    ROM_SysCtlPeripheralEnable(SDC_SSI_SYSCTL_PERIPH); // Включение SPI
    ROM_SysCtlPeripheralEnable(SDC_GPIO_SYSCTL_PERIPH); // Включение GPIO
    ...
    ROM_SSIConfigSetExpClk(SDC_SSI_BASE, ROM_SysCtlClockGet(),
                           SSI_FRF_MOTO_MODE_0, SSI_MODE_MASTER, 400000, 8); //400 kHz speed
...
void set_max_speed(void)
{
    ...
    /* Set the maximum speed as half the system clock, with a max of 20.0 MHz. */
    /* 12.5 MHz is stability speed */
